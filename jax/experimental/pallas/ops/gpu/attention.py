...
def mha_forward_kernel(
  ...
  head_mask = (jnp.arange(block_d) < head_dim)[None, :]
  ...
  def body(start_k, carry):
    ...
    v = pl.load(v_ref, (curr_k_slice, pl.dslice(block_d)), mask=head_mask)
...
def mha(
  ...
  in_specs = [
      ...
      pl.BlockSpec(
          (None, kv_seq_len, None, block_d), lambda _, j, k: (j, 0, k, 0)
      ),
      pl.BlockSpec(
          (None, kv_seq_len, None, block_d), lambda _, j, k: (j, 0, k, 0)
      ),
...
def _mha_backward(sm_scale: float, causal: bool, block_sizes: BlockSizes,
  ...
  if backward_pass_impl == "xla":
    ...
    dq, dk, dv = pl.pallas_call(
        ...
        out_specs=[
            pl.BlockSpec(
                (None, block_q_dq, None, block_d),
                lambda i, j, k: (i, k, j, 0),  # dq
            ),
            pl.BlockSpec(
                (None, block_kv_dkv, None, block_d),
